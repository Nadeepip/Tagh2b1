...
window.Portal = {
  ...
  async authenticate(callbacks) {
    ...
    if (this.studentId && this.token && this.homeworkId && this.gameId) {
      ...
      try {
        ...
        this.showLoadingScreen(true, 'در حال احراز هویت و دریافت نمایه شما...', '70%');
  ...
  async submitProgress(score, callbacks) {
    if (callbacks) this._lastSubmitCallbacks = callbacks;
    const needsSubmitKey = `portal_needs_submit_${this.gameId}`;
    const pendingScoreKey = `portal_pending_score_${this.gameId}`;

    if (score !== null) {
      localStorage.setItem(needsSubmitKey, 'true');
      localStorage.setItem(pendingScoreKey, score.toString());
    }
    ...
    try {
      ...
      const pendingScore = parseInt(localStorage.getItem(pendingScoreKey)) || 0;
      const currentPlays = this.previousPlays + 1;
      const currentStars = this.previousStars + (pendingScore === 10 ? 1 : 0);
      ...
      if (submitBadge) submitBadge.style.display = 'block';
      ...
      if (statusText) {
        ...
        <span style="font-size: 11pt; font-weight: normal; color: #666; display: block; margin-top: 5px; line-height:1.7;">
            <b>علت خطا:</b> اتصال اینترنت شما قطع است یا سرور پرتال موقتاً پاسخ نمی‌دهد.<br/>
            <b>راهنما:</b> لطفا وای‌فای یا داده همراه گوشی خود را روشن کرده و مجدداً روی دکمه «تلاش مجدد برای ارسال» بزنید. پیشرفت شما تا زمان ارسال در حافظه گوشی محفوظ خواهد بود.
        </span>`;
      }
